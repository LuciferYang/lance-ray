...
class TestCompactDatabase:
    ...
    def _mock_two_table_namespace(self) -> MagicMock:
        mock_response = MagicMock()
    ...
    def test_compact_database_continue_on_error_records_failures(self) -> None:
        """continue_on_error=True records per-table failures and keeps going."""
        mock_namespace = self._mock_two_table_namespace()

        def fake_compact_files(*, table_id: list[str], **kwargs: Any) -> Any:
    ...
    def test_compact_database_aborts_on_first_error_by_default(self) -> None:
